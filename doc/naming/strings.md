...
| 前缀            | 含义 |
|----------------|-------------------------------------------------------------|
| toast          | Android特有的Toast提示信息
| text           | 比较短的文字（通常为1-3个单词，或者1-6个汉字）
| message        | 比较短的信息（在手机屏幕中不需换行显示）
| description    | 比较长的信息（在手机屏幕中通常需要换行显示）
| paragraph      | 段落（一段较多的文字，在手机屏幕中通常需要换行显示）
| content        | 内容（大段的文本，甚至可以在手机屏幕上整页显示）
| title          | 标题（页面的标题）
| head           | 标题（段落的标题）

| 前缀            | 含义 |
|----------------|-------------------------------------------------------------|
| hint           | 表单页面中可编辑控件内的提示文字
| lable          | 表单页面中可编辑控件前面的标题文字
| edit_hint      | 编辑框的提示信息
| button         | 按钮
| dialog_button  | 对话框按钮
| dialog_lable   | 对话框表单的可编辑控件前面的标题文字
...
| 后缀            | 含义 |
|----------------|-------------------------------------------------------------|
| format         | 表示格式的字符串，比如“%d年%d月”，“我叫%s，来自%s”
...
| 前缀                    | 含义 |
|------------------------|-----------------------------------------------------|
| dialog\_button\_ok     | 对话框的“确定”按钮
| dialog\_button\_cancel | 对话框的“取消”按钮